...
BEGIN
  ...
  DROP POLICY IF EXISTS "Allow all for objects" ON objects;
  DROP POLICY IF EXISTS "Allow all for entities" ON entities;
...
CREATE POLICY "Allow all for objects" ON objects FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Allow all for entities" ON entities FOR ALL USING (true) WITH CHECK (true);
